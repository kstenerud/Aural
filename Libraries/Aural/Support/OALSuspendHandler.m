...
- (void) removeSuspendListener:(id<OALSuspendListener>) listener
{
	@synchronized(self)
	{
		NSUInteger idx = [listeners indexOfObject:listener];
		if(NSNotFound != idx)
		{
			[listeners removeObjectAtIndex:idx];
			[manualSuspendStates removeObjectAtIndex:idx];
		}
	}
}
...
- (void) setManuallySuspended:(bool) value
{
	...
	@synchronized(self)
	{
		...
		if(value)
		{
			...
			for(NSUInteger idx = 0; idx < numListeners; idx++)
			{
				id<OALSuspendListener> listener = [listeners objectAtIndex:idx];
				
				// Record whether they were already suspended or not
				bool alreadySuspended = listener.manuallySuspended;
				if(alreadySuspended != [[manualSuspendStates objectAtIndex:idx] boolValue])
				{
					[manualSuspendStates replaceObjectAtIndex:idx withObject:[NSNumber numberWithBool:alreadySuspended]];
				}
		...
		if(!value)
		{
			for(int idx = (int)[listeners count] - 1; idx >= 0; idx--)
			{
				id<OALSuspendListener> listener = [listeners objectAtIndex:idx];
				
				bool alreadySuspended = [[manualSuspendStates objectAtIndex:idx] boolValue];
